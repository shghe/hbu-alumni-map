...
3.0.0 已从微信云托管迁移到腾讯轻量服务器，并完成 Docker 化部署。
...
- Docker 镜像：`hbu-alumni-api:3.0.0`
...
- Git tag：`v3.0.0` 已在本地创建；推送 GitHub 时本机网络连接 GitHub 失败，网络恢复后执行 `git push origin v3.0.0`

已验证：

- `http://api.aluhomemap.top/api/health` 正常
- Nginx `/api/` 反代到 Docker 容器监听的 `127.0.0.1:3002`
- Docker 容器 `hbu-alumni-api` 使用镜像 `hbu-alumni-api:3.0.0`，状态 `Up`
...
- 图片流验证返回 `200 image/jpeg`
- 视频 Range 验证返回 `206 Partial Content`
...
校友地图后端已改为 Docker Compose 运行：

```text
/home/ubuntu/hbu-alumni-map-api
├── Dockerfile
├── docker-compose.yml
├── .env
├── server.js
...
- 后端需要访问宿主机本机 MySQL：`127.0.0.1:3306`
- Nginx 已稳定反代到 `127.0.0.1:3002`
- 避免额外处理容器访问宿主机数据库的网关地址

注意：后续新项目不要复用 `3002` 端口。
...
```text
/home/ubuntu/project-management
```
...
```text
/opt/alumni-map
/opt/project-management
/opt/nginx
```

不建议把两个项目文件混在同一个目录。推荐约定：

- 一个项目一个目录
- 一个项目一个 `docker-compose.yml`
- 一个项目一个固定容器名前缀
- 一个项目一个内部端口
- 统一由 Nginx 按域名或路径反代
...
```text
校友地图 API：127.0.0.1:3002
项目管理系统 API：127.0.0.1:3003
项目管理系统前端：127.0.0.1:8080 或由 Nginx 直接托管
```

Nginx 继续占用公网 `80/443`，业务容器只监听本机端口，不直接暴露数据库和内部服务。
...
部署前按 `cloudrun-service/.env.example` 准备 `.env`，其中两项缺失会导致服务拒绝启动或媒体无法访问：

```text
JWT_SECRET           # 随机长字符串，缺失时后端拒绝启动
MEDIA_STREAM_SECRET  # 随机长字符串，缺失时媒体签名全部校验失败
CORS_ALLOW_ORIGIN    # 留空则不返回跨域头；如浏览器调试需要再填来源域名
```

多实例部署时 `JWT_SECRET` 和 `MEDIA_STREAM_SECRET` 必须在所有实例保持一致。
...
当前 `utils/config.js` 固定使用 HTTPS：

```js
const API_BASE_URL = HTTPS_API_BASE_URL // https://api.aluhomemap.top
```
...
`HTTP_API_BASE_URL` 仅作为兜底常量保留，不再用于运行路径。如后续需要按 `getEnvVersion()` 区分环境，再切换逻辑。
...
```bash
sudo docker images hbu-alumni-api
```

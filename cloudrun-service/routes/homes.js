const express = require('express')
const router = express.Router()
const { pool } = require('../utils/db')
const { signUrls } = require('../utils/cos')

async function loadHomeRelations(homeIds) {
  if (!homeIds.length) return { photosByHome: new Map(), servicesByHome: new Map() }

  const placeholders = homeIds.map(() => '?').join(',')
  const [photos] = await pool.query(
    `SELECT home_id, url FROM home_photos WHERE home_id IN (${placeholders}) ORDER BY home_id, sort`,
    homeIds
  )
  const [services] = await pool.query(
    `SELECT home_id, service FROM home_services WHERE home_id IN (${placeholders})`,
    homeIds
  )

  const photosByHome = new Map()
  const servicesByHome = new Map()
  photos.forEach((photo) => {
    if (!photo.url) return
    if (!photosByHome.has(photo.home_id)) photosByHome.set(photo.home_id, [])
    photosByHome.get(photo.home_id).push(photo.url)
  })
  services.forEach((service) => {
    if (!servicesByHome.has(service.home_id)) servicesByHome.set(service.home_id, [])
    servicesByHome.get(service.home_id).push(service.service)
  })

  return { photosByHome, servicesByHome }
}

async function serializeHome(home, photos = [], services = []) {
  const signedPhotos = await signUrls(photos)
  const signedVideo = home.video ? (await signUrls([home.video]))[0] : ''
  const signedPoster = home.video_poster ? (await signUrls([home.video_poster]))[0] : ''

  return {
    ...home,
    _id: String(home.id),
    dbId: String(home.id),
    contactName: home.contact_name,
    video: signedVideo,
    videoPoster: signedPoster,
    photos: signedPhotos,
    services
  }
}

// GET /api/homes — 所有校友之家列表
router.get('/', async (req, res) => {
  try {
    const [homes] = await pool.execute(
      'SELECT * FROM alumni_homes ORDER BY id'
    )

    const { photosByHome, servicesByHome } = await loadHomeRelations(homes.map(home => home.id))
    const result = await Promise.all(homes.map(home => serializeHome(
      home,
      photosByHome.get(home.id) || [],
      servicesByHome.get(home.id) || []
    )))

    res.json({ code: 0, data: result })
  } catch (err) {
    console.error('GET /homes error:', err)
    res.status(500).json({ code: 500, message: '加载失败', error: err.message })
  }
})

// GET /api/homes/:id — 单个详情
router.get('/:id', async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10)
    if (isNaN(id)) return res.status(400).json({ code: 400, message: 'id 无效' })

    const [[home]] = await pool.execute(
      'SELECT * FROM alumni_homes WHERE id = ?', [id]
    )
    if (!home) return res.status(404).json({ code: 404, message: '未找到' })

    const [photos] = await pool.execute(
      'SELECT url FROM home_photos WHERE home_id = ? ORDER BY sort', [id]
    )
    const [services] = await pool.execute(
      'SELECT service FROM home_services WHERE home_id = ?', [id]
    )

    const data = await serializeHome(
      home,
      photos.map(p => p.url).filter(Boolean),
      services.map(s => s.service)
    )

    res.json({
      code: 0,
      data: {
        ...data,
        id: String(home.id),
      }
    })
  } catch (err) {
    console.error('GET /homes/:id error:', err)
    res.status(500).json({ code: 500, message: '加载失败' })
  }
})

module.exports = router
